  ...
  render() {
    ...
    if (isLoaded) {
      content = (
        <div>
          ...
          <Select options={countries} onChange={this.handleSourceChange} />

          <label className="block mb-3 mt-2">Cel trasy</label>
          <Select options={countries} onChange={this.handleDestinationChange} />
        </div>
      );
    } else {
    ...
    return (
      <div className="mr-6 form-wrap flex-1 p-6 bg-gray-200 rounded shadow-sm self-center">
        <p className="text-center bg-white -m-6 mb-6 p-6 font-bold rounded rounded-b-none border-b border-gray-400">
          Podaj informacje
        </p>
